...
from .hhrequest import HHRequest

from .hhparser_description import HHParserDescription
from .hhparser_key_skills import HHParserKeySkills
from .hhparser_salary import HHParserSalary
...
def process_request(db_path, file_folder, row_request):
    ...
    o_pars_description = HHParserDescription()
    o_pars_salary = HHParserSalary()
    o_pars_key_skills = HHParserKeySkills()
    ...
    o_hhrequest = HHRequest(o_pars_description)
    ...
    if not l_urls:
        ...
        file_name: str = file_folder + "/" + str(
            row_request.user_id) + "-" + "".join(
            row_request.text_request) + "-" \
                         + row_request.region + "-" + time.strftime(
...
def add_skills(sum_skills: dict, skills_to_add: list):
    #print(f"skills_to_add - {skills_to_add}")
...
def update_request(db_session, row_request, file_name, number_vacancies):
    ...
    db_session.query(Request).filter(Request.id == row_request.id). \
        update({Request.file_name: file_name,
                Request.vacancy_number: number_vacancies, Request.status: 2})
    db_session.commit()
...
def update_status(db_session, row_request, status):
    ...
    if status == 1:
        ...
        db_session.query(Request).filter(Request.id == row_request.id). \
            update({Request.status: status, Request.vacancy_number: 0,
